    ...
    try {
        if (req.method === 'POST') {
            authenticate(req, res, async () => {
                ...
                const { kategori_id, customer_id, sumber, type_motor, warna_motor, model_motor, jenis_pembayaran, jenis_service, jadwal_service, jenis_sparepart, nama_sparepart, jenis_keluhan, jenis_informasi, keterangan, cabang_id, crm_id, tujuan_user, status_kontak, paginate, page, limit } = req.body;
                ...
                if (paginate){
                    await Pesanan.allWithFilter(filters, page, limit).then((result) => {
                        if (result.error) {
                    ...
                    await Pesanan.allWithFilter(filters).then((result) => {
                        if (result.error) {
                            return res.status(400).json({ status: 'error', message: result.error });
                        } else {
                            return res.status(200).json({ status: 'success', message: 'Fetching data success', data: result });
