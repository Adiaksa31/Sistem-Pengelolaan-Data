    ...
    try {
        if (req.method === 'POST') {
            authenticate(req, res, async () => {
                ...
                const { id, kategori_id, customer_id, sumber, type_motor, warna_motor, model_motor, jenis_pembayaran, jenis_service, jadwal_service, jenis_sparepart, nama_sparepart, jenis_keluhan, jenis_informasi, keterangan, cabang_id, crm_id, tujuan_user, status_kontak } = req.body;
                ...
                await Pesanan.update(
                    ...
                    if (result.error) {
                        ...
                        return res.status(200).json({ status: 'success', message: 'Pesanan updated successfully' });
                    }
                }).catch((err) => {
